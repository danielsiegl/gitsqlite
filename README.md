...
📖 **For more usage scenarios and workflows, see [SCENARIOS.md](SCENARIOS.md)**
...
📖 **For more scenarios including team collaboration and CI/CD integration, see [SCENARIOS.md](SCENARIOS.md)**
...
### Core Documentation
- **[SCENARIOS.md](SCENARIOS.md)** - Real-world usage scenarios and best practices for different use cases
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** - Detailed troubleshooting guide for diagnosing and fixing issues
- **[log.md](log.md)** - Comprehensive logging documentation
...
- **`sqlite_sequence` table**: Content can change outside of your edits. This table is excluded from dumps as it's automatically regenerated by SQLite. Auto-increment continues from the highest ID in the table.
- **Large databases**: Databases over 100MB may be slow to convert. Consider using schema/data separation (`-data-only -schema` flags) for better performance and cleaner diffs.
- **Temporary files**: Written to the system temp directory (usually `/tmp` on Linux/macOS, `%TEMP%` on Windows). Ensure adequate disk space is available.

### Potential Issues

- **Merge conflicts**: While gitsqlite enables text-based diffs, complex database merges may require specialized tools. See the [⚠️ Important Notice](#️-important-notice-database-merging) section above.
- **Schema changes during collaboration**: Multiple developers changing schema simultaneously can lead to conflicts. Coordinate schema changes or use feature branches.
- **Float precision**: Different platforms may display floats differently. Use `-float-precision` flag for consistent output (default: 9 digits).
- **Binary data (BLOBs)**: Large BLOBs increase SQL file size significantly. Consider storing large binary files separately from the database.
- **Very large tables**: Tables with millions of rows create large SQL dumps. Consider archiving historical data or using database partitioning.
- **Transaction logs**: Write-ahead logging (WAL) files are not processed by gitsqlite. Ensure database is in DELETE mode or fully checkpointed before conversion.
- **Encrypted databases**: SQLite encryption extensions (like SQLCipher) are not supported. Database must be in plain SQLite format.
- **Virtual tables**: Some virtual table types may not round-trip correctly. Test thoroughly if using FTS, R*Tree, or custom virtual tables.

### Platform-Specific Considerations

- **Windows**: Paths with spaces should be quoted in Git config. Line endings are normalized to LF in SQL output.
- **macOS**: Homebrew sqlite3 vs system sqlite3 may produce slightly different output. Use consistent version across team.
- **Linux**: Different distributions may have different default temp directory locations and permissions.
